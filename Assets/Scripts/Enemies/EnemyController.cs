    ...
    public class EnemyController : MonoBehaviour
    {
        ...
        private void HandleWander(float speed)
        {
            ...
            if (isNearEdge)
            {
                ...
                //Bias the new destination away from the edge
                Vector3 safeDirection = awayFromEdge * 8f;
                Vector3 safePosition = transform.position + safeDirection;

                NavMeshHit safeHit;
                if (NavMesh.SamplePosition(safePosition, out safeHit, 10f, NavMesh.AllAreas))
                {
                    activeData.wanderDestination = safeHit.position;
        ...
        private bool CheckForEdge(out Vector3 awayFromEdge)
        {
            ...
            if (NavMesh.FindClosestEdge(transform.position, out hit, NavMesh.AllAreas))
            {
                ...
                if (distanceToEdge < edgeSlowdownRadius)
                {
                    float slowFacctor = distanceToEdge / edgeSlowdownRadius;
                    agent.speed *= slowFacctor;
                }
            }
